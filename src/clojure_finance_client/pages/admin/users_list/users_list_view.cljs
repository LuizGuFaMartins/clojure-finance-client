        ...
        [:td {:class "px-6 py-4 text-center"}
         [:div {:class "flex justify-center gap-3"}
          ...
          [:button {:class "text-blue-400 hover:text-blue-300 p-1 transition"
                    :title "Editar"
                    :on-click #(rf/dispatch [:admin/edit-user id])}
           [:svg {:class "w-5 h-5" :fill "none" :stroke "currentColor" :viewBox "0 0 24 24"}
            [:path {:stroke-linecap "round" :stroke-linejoin "round" :stroke-width "2" :d "M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"}]]]
          ;; Botão Excluir
          [:button {:class "text-red-400 hover:text-red-300 p-1 transition"
                    :title "Excluir"
                    :on-click #(when (js/confirm "Deseja realmente excluir este usuário?")
                                 (rf/dispatch [:admin/delete-user id]))}
           [:svg {:class "w-5 h-5" :fill "none" :stroke "currentColor" :viewBox "0 0 24 24"}
          ...
          [:div {:class "p-8 border-b border-slate-800 bg-slate-900/50"}
           [:div {:class "flex flex-col md:flex-row justify-between items-start md:items-center gap-4"}
            ...
            [:div {:class "flex gap-3"}
             ...
             ;; Botão Ir para Usuário
             [:button
              {:class "bg-slate-800 text-zinc-300 px-4 py-2 rounded-lg hover:bg-slate-700 transition"
               :on-click #(rfe/push-state :customer)}
              "Área do Cliente"]]]]
           ...
           [:div {:class "flex justify-between items-center mb-6"}
            ...
            [:button
             {:class "flex items-center gap-2 bg-indigo-600 text-white px-5 py-2.5 rounded-xl font-semibold hover:bg-indigo-500 shadow-lg shadow-indigo-900/20 transition active:scale-95"
              :on-click #(rf/dispatch [:admin/new-user-modal])}
             [:svg {:class "w-5 h-5" :fill "none" :stroke "currentColor" :viewBox "0 0 24 24"}
